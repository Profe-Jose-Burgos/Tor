...
#------------------------------Vamos a capturar el rostro-----------------------------------------------------
    ...
    while(True):
        ...
        if cv2.waitKey(1) == 27:            #Cuando oprimamos "Escape" toma la captura
            break
...
def pantalla_principal():
    ...
    pantalla.title("Samsung Campus Innovation")       #Asignamos el titulo de la pantalla
    Label(text = "Hackatón", bg = "gray", width = "300", height = "2", font = ("Verdana", 13)).pack() #Asignamos caracteristicas de la ventana
...
get_ipython().system('pip install firebase-admin')
...
def detect_faces(image):
    ...
    if len(faces) > 0:
        send_notification("Se ha detectado un rostro")
...
# se debe enviar un mensaje para el token de registro

response = messaging.send(message)
# respuesta a el mensaje
